...
**class Form<T> extends Component** - предназначен для работы с формами, обеспечивая управление состоянием формы, на базе интерфейса IForm. Отвечает за отображение ошибок и генерацию событий при изменении значений полей.    
Включает методы:
- `onInputChange` - вызывается при изменении значения в поле формы и генерирует событие
- `set valid` - устанавливает состояние кнопки отправки формы (disabled или не disabled), в зависимости от переданного значения     
- `set errors` - устанавливает текст ошибки для отображения в элементе      
- `render` - принимает объект состояния формы (valid, errors и другие поля) и обновляет состояние компонента, а также переданные поля.  
...
Включает методы:   
- `set button` - устанавливает название кнопки карточки 
- `set id` - устаналивает индентификатор карточки   
- `get id` - получает индентификатор карточки   
- `set description` - устаналивает описание карточки    
- `get description` - получает описание карточки    
- `set image` - устанавливает изображение карточки   
- `set title` - устанавливает название карточки 
- `get title` - получает название карточки  
- `set category` - устанавливает категорию товара карточки  
- `get categoty` - получает категорию товара карточки   
- `set price` - устанавливает стоимость товара карточки 
- `get price` - получает стоимость товара карточки  

**class BasketItemd extends Card** - расширяет класс Card. Отвечает за отображение индекса товара в корзине, и кнопки удаления товара из корзины. 
Включает методы:    
- `setIndex` - присваивает индекс товара в корзине

**class FormPayment extends Form** - форма заполнения данных клиентом, включает выбор метода оплаты и поле ввода адреса   
Включает методы:    
- `set address` - устаналивает адресс
- `togglePayment` - переключение метода оплаты
- `cancelPayment` - вспомогательная функция для togglePayment

**class FormContacts extends Form** - форма заполнения данных клиентом, включает поля ввода email и phone
Включает методы:    
